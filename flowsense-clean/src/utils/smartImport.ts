...
const MERCHANT_CATEGORIES = new Map(
  MERCHANT_RULES.map((rule) => [rule.merchant, rule.category])
);
...
function matchMerchantRule(rawTitle: string) {
...
function inferPaymentMethod(rawTitle: string) {
  const cleaned = normalizePattern(rawTitle);

  for (const method of PAYMENT_METHOD_RULES) {
...
function inferMerchant(rawTitle: string) {
...
function classifyTransaction(rawValue: string) {
...
function getConfidenceLabel(rawTitle: string, merchant: string, category: string): {
  label: ConfidenceLabel;
  score: number;
} {
  const merchantRule = matchMerchantRule(rawTitle);

  if (merchantRule && merchantRule.merchant.toLowerCase() === normalizePattern(merchant)) {
    return {
      label: "High",
      score: 0.95,
    };
...
function normalizeImportedTransaction(rawTitle: string): {
  title: string;
  ...
  return {
    title: merchant || titleCase(rawTitle) || "Imported Transaction",
